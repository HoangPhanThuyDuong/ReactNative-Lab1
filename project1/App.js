...
export default function App() {
  ...
  const styles = StyleSheet.create({
    ...
    themeButton: {
      ...
      bottom: '5%',
      margin: 15,
      backgroundColor: darkMode ? 'green' : 'lightgreen',
      alignItems: 'center',
    ...
    <View>
      <View style={styles.results}>
        <TouchableOpacity style={styles.themeButton}>
          <Icon 
            name={darkMode ? 'light-up' : 'moon'} size={24} 
      ...
      <View style={styles.buttons}>
        {buttons.map((btn) =>
          btn === '=' || btn === '/' || btn === '*' || btn === '-' || btn === '+' ?
            <TouchableOpacity key={btn} style={[styles.button, { backgroundColor: 'forestgreen' }]} onPress={() => handleInput(btn)}>
